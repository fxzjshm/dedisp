...
bool malloc_device(T*& addr, gpu_size_t count) {
	hipError_t error = hipMalloc((void**)&addr, count*sizeof(T));
	if( error != hipSuccess ) {
		return false;
	}
	return true;
...
bool copy_host_to_symbol(const char* symbol, const T* src,
						 gpu_size_t count, hipStream_t stream=0) {
	// TODO: Can't use Async versions unless host memory is pinned!
	hipMemcpyToSymbol/*Async*/(symbol, src,
							count * sizeof(T),
...
bool copy_device_to_symbol(/*const char**/U symbol, const T* src,
						   gpu_size_t count, hipStream_t stream=0) {
	hipMemcpyToSymbolAsync(HIP_SYMBOL(symbol), src,
							count * sizeof(T),
...
bool copy_host_to_device_2d(T* dst, gpu_size_t dst_stride,
	...
	// TODO: Can't use Async versions unless host memory is pinned!
	hipMemcpy2D/*Async*/(dst, dst_stride,//*sizeof(T),
...
bool copy_device_to_host_2d(T* dst, gpu_size_t dst_stride,
                            ...
                            hipStream_t stream=0) {
	// TODO: Can't use Async versions unless host memory is pinned!
	hipMemcpy2D/*Async*/(dst, dst_stride,
	                      src, src_stride,
						  width_bytes, height,
						  hipMemcpyDeviceToHost/*, stream*/);
	//#ifdef DEDISP_DEBUG
