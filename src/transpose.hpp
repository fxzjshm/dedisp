...
void transpose_kernel(const T* in,
					  ...
					  gpu_size_t log2_gridDim_y, sycl::nd_item<3> item_ct1,
					  dpct::accessor<T, dpct::local, 2> tile)
{
	...
	if( width == height ) {
                blockIdx_y = item_ct1.get_group(2);
                if( !GRID_IS_POW2 ) {
                        blockIdx_x =
                            (item_ct1.get_group(2) + item_ct1.get_group(1)) %
                            item_ct1.get_group_range(2);
                }
		else {
                        blockIdx_x =
                            (item_ct1.get_group(2) + item_ct1.get_group(1)) &
                            (item_ct1.get_group_range(2) - 1);
                }
	}
	else {
                gpu_size_t bid =
	...
	if( blockIdx_x >= block_count_x ||
        ...
        gpu_size_t index_in_x =
            blockIdx_x * Transpose<T>::TILE_DIM + item_ct1.get_local_id(2);
                             ...
                             sycl::queue *stream)
{
	...
	// Partition the grid into chunks that the GPU can accept at once
        for (size_t block_y_offset = 0; block_y_offset < tot_block_count[1];
             block_y_offset += max_grid_dim) {

                sycl::range<3> block_count(1, 1, 1);

                // Handle the possibly incomplete final grid
                block_count[1] = min(max_grid_dim, tot_block_count[1] - block_y_offset);

                for (size_t block_x_offset = 0;
                     block_x_offset < tot_block_count[2];
                     block_x_offset += max_grid_dim) {

                        // Handle the possibly incomplete final grid
                        block_count[2] = min(max_grid_dim, tot_block_count[2] - block_x_offset);
			...
			size_t h = min(max_grid_dim*TILE_DIM, height-y_offset);
                        ...
                        sycl::range<3> block(1, BLOCK_ROWS, TILE_DIM);
			...
			if( round_grid_to_pow2 ) {
                                sycl::range<3> grid(
                                    1, round_up_pow2(block_count[1]),
                                    round_up_pow2(block_count[2]));
                                // Run the CUDA kernel
                                /*
                                DPCT1049:8: The workgroup size passed to the
                                SYCL kernel may exceed the limit. To get the
                                device limit, query
                                info::device::max_work_group_size. Adjust the
                                workgroup size if needed.
                                */
                                stream->submit([&](sycl::handler &cgh) {
                                        // ranges used for accessors to device
                                        // memory
                                        sycl::range<2> tile_range_ct1(
                                            Transpose<T>::TILE_DIM,
                                ...
                                sycl::range<3> grid(1, block_count[1], block_count[2]);
                                // Run the CUDA kernel
                                /*
                                DPCT1049:9: The workgroup size passed to the
                                SYCL kernel may exceed the limit. To get the
                                device limit, query
                                info::device::max_work_group_size. Adjust the
                                workgroup size if needed.
