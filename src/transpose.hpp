...
void transpose_kernel(const T* in,
					  ...
					  sycl::accessor<T, 2, sycl::access_mode::read_write, sycl::access::target::local> tile)
{
	...
	if( width == height ) {
		...
		if( !GRID_IS_POW2 ) {
			blockIdx_x = (item_ct1.get_group(0)+item_ct1.get_group(1)) % item_ct1.get_group_range(0);
		}
		else {
			blockIdx_x = (item_ct1.get_group(0)+item_ct1.get_group(1)) & (item_ct1.get_group_range(0)-1);
		}
	}
	else {
		gpu_size_t bid = item_ct1.get_group(0) + item_ct1.get_group_range(0)*item_ct1.get_group(1);
		if( !GRID_IS_POW2 ) {
			blockIdx_y = bid % item_ct1.get_group_range(1);
			blockIdx_x = ((bid / item_ct1.get_group_range(1)) + blockIdx_y) % item_ct1.get_group_range(0);
		}
		else {
			blockIdx_y = bid & (item_ct1.get_group_range(1)-1);
			blockIdx_x = ((bid >> log2_gridDim_y) + blockIdx_y) & (item_ct1.get_group_range(0)-1);
		}
	}
	...
	gpu_size_t index_in_x = blockIdx_x * Transpose<T>::TILE_DIM + item_ct1.get_local_id(0);
	gpu_size_t index_in_y = blockIdx_y * Transpose<T>::TILE_DIM + item_ct1.get_local_id(1);
...
#pragma unroll
	for( gpu_size_t i=0; i<Transpose<T>::TILE_DIM; i+=Transpose<T>::BLOCK_ROWS ) {
		...
		if( index_in_x < width && index_in_y+i < height )
		        tile[item_ct1.get_local_id(1)+i][item_ct1.get_local_id(0)] = in[index_in+i*in_stride];
	}
	...
	*/
	...
	gpu_size_t index_out_x = blockIdx_y * Transpose<T>::TILE_DIM + item_ct1.get_local_id(0);
	// Avoid excess threads
	if( index_out_x >= height ) return;
	gpu_size_t index_out_y = blockIdx_x * Transpose<T>::TILE_DIM + item_ct1.get_local_id(1);
...
#pragma unroll
	for( gpu_size_t i=0; i<Transpose<T>::TILE_DIM; i+=Transpose<T>::BLOCK_ROWS ) {
		...
		if( index_out_y+i < width ) {
			out[index_out+i*out_stride] = tile[item_ct1.get_local_id(0)][item_ct1.get_local_id(1)+i];
		}
	}
}
							 ...
							 sycl::queue* stream)
{
	...
	sycl::range<3> tot_block_count((width-1)  / TILE_DIM + 1, (height-1) / TILE_DIM + 1, 1);
	...
	for( size_t block_y_offset = 0;
		...
		for( size_t block_x_offset = 0;
			...
			if( round_grid_to_pow2 ) {
				sycl::range<3> grid(round_up_pow2(block_count[0]), round_up_pow2(block_count[1]), 1);
				// Run the CUDA kernel
				stream->submit(
				  [&](sycl::handler &cgh) {
				    sycl::accessor<T, 2, sycl::access_mode::read_write, sycl::access::target::local> tile_acc_ct1(sycl::range<2>(Transpose<T>::TILE_DIM, Transpose<T>::TILE_DIM+1), cgh);
				    ...
				    cgh.parallel_for(
				      ...
				      [=](sycl::nd_item<3> item_ct1) {
				        transpose_kernel<true>(in + in_offset, w, h, in_stride, out_stride, out + out_offset, block_count[0], block_count[1], log2_grid_y_ct8, item_ct1, tile_acc_ct1);
				      });
				  });
			}
			else {
				...
				stream->submit(
				  [&](sycl::handler &cgh) {
				    ...
				    cgh.parallel_for(
				      sycl::nd_range<3>(grid * block, block), 
				      [=](sycl::nd_item<3> item_ct1) {
				        transpose_kernel<false>(in + in_offset, w, h, in_stride, out_stride, out + out_offset, block_count[0], block_count[1], log2_grid_y_ct8, item_ct1, tile_acc_ct1);
				      });
				  });
			}
